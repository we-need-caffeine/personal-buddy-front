  ...
  return (
    <S.EventWrapper>
      ...
      <S.BannerSliderWrapper>
        ...
        >
          {banners.map((event, i) => {
            const parts = event.thumbnailUrl?.split('/'); // 이미지 경로를 슬래시를 기준으로 나눠서 배열로 바꾼다.
            const filePath = parts?.slice(0, -1).join('/'); // 마지막 요소(파일 이름)를 제외한 나머지를 다시 슬래시로 이어붙인다.
            const fileName = parts?.slice(-1)[0]; // 마지막 요소만 뽑는다 --> 파일명
