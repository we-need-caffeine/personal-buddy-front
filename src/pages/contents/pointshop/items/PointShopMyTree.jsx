...
const PointShopMyTree = () => {
  ...
  const paginatedItems = items.filter(item => item.itemType === "나무").slice(
    (currentPage - 1) * 24,
    currentPage * 24
  );
    ...
    <S.ItemCardListBox>
      ...
      <S.PaginationWrapper>
        <Pagination
          ...
          totalPages={Math.ceil(items.filter(item => item.itemType === "나무").length / 24)}
...
export default PointShopMyTree;